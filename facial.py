...
def registro_facial():
    ...
    while True:
        ...
        for rostro in rostros:
            ...
            cv2.rectangle(frame, (x, y), (x + ancho, y + alto), (0, 255, 0), 2)  # Dibuja un rectángulo verde
...
def login_facial():
    cap = cv2.VideoCapture(0)  # Elegimos la cámara para la detección
    detector = MTCNN()  # Inicializamos el detector de rostros
    ...
    while True:
        ...
        for rostro in rostros:
            x, y, ancho, alto = rostro['box']
            cv2.rectangle(frame, (x, y), (x + ancho, y + alto), (0, 255, 0), 2)  # Dibuja un rectángulo verde alrededor del rostro

        cv2.imshow('Login Facial', frame)  # Mostramos el video en pantalla

        if cv2.waitKey(1) == 27:  # Si se presiona "Escape", se rompe el ciclo
            break

    usuario_login = verificacion_usuario.get()  # Guardamos el nombre del usuario
    cv2.imwrite(usuario_login + "LOG.jpg", frame)  # Guardamos la última captura del video como imagen
...
def pantalla_principal():
    ...
    pantalla.title("PRUEBA1")       #Asignamos el titulo de la pantalla
    Label(text = "DETECTOR FACIAL", bg = "gray", width = "300", height = "2", font = ("Verdana", 13)).pack() #Asignamos caracteristicas de la ventana
